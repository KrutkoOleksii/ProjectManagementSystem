...
CREATE TABLE hw2.developers (
  ...
  salary INT NOT NULL,
  PRIMARY KEY (id),
  FOREIGN KEY (company_id) REFERENCES  hw2.companies(id) ON DELETE SET NULL ON UPDATE RESTRICT
);
...
CREATE TABLE hw2.projects (
  ...
  PRIMARY KEY (id),
  FOREIGN KEY (company_id) REFERENCES  hw2.companies(id) ON DELETE SET NULL ON UPDATE RESTRICT,
  FOREIGN KEY (customer_id) REFERENCES  hw2.customers(id) ON DELETE SET NULL ON UPDATE RESTRICT
);
...
CREATE TABLE hw2.developer_skill (
  ...
  FOREIGN KEY (skill_id) REFERENCES  hw2.skills(id) ON DELETE CASCADE ON UPDATE RESTRICT,
  FOREIGN KEY (developer_id) REFERENCES  hw2.developers(id) ON DELETE CASCADE ON UPDATE RESTRICT
);
...
CREATE TABLE hw2.developer_project (
  ...
  FOREIGN KEY (project_id) REFERENCES  hw2.projects(id) ON DELETE CASCADE ON UPDATE RESTRICT,
  FOREIGN KEY (developer_id) REFERENCES  hw2.developers(id) ON DELETE CASCADE ON UPDATE RESTRICT
);
